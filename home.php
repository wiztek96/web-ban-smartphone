...
<title>Điện thoại IT Plus</title>
</head>
   ...
   <ul>
       ...
       <!-- Chia trang m-->
        <?php 
	   		$page=$_GET["page"];
			$maxitems=9;   
	   		$start=((int)$page-1)*$maxitems;
			$query = mysqli_query($conn, "select * from sanpham LIMIT ".$start.",".$maxitems."");
	...
	margin:5px;"><img src="Images/<?php echo $sp['idhang'] ?>" style="width:127px; height:127px;" /></p>
			<p style="padding: 6px"><?php echo $sp['tensanpham'] ?></p>
    ...
    <p style="margin: 643px;">
    <?php 
	   		$page=$_GET["page"];
			if ($page == 1){
           echo '<p align="right"><strong><a href="home.php?page='.($page+1).'"> (Tiếp) >| </a></strong></p>  ';
			}
				if ($page > 1){
           echo '<p align="right"><strong><a href="home.php?page='.($page-1).'"> (Lùi) <| </a></strong></p> ';
            }
		?>
		</p>
		 </div>
